 ...
 ***
 *
 * @brief Chameleon zrecgemm_R testing
 *
 * @version 1.0.0
...
void
print_descriptor_rec_R( CHAM_desc_t *desc, char *name, int lvl )
{
    int i;
    ...
    for ( i=0; i<desc->lmt*desc->lnt; i++ ) {
        ...
        if ( t.format == CHAMELEON_TILE_DESC ) {
            ...
            print_descriptor_rec_R( (CHAM_desc_t*)(t.mat), NULL, lvl+1 );
        }
        else if ( t.format == CHAMELEON_TILE_FULLRANK ) {
            /* fprintf(stderr, "[LVL%d] tile-%d (%d, %d)\n", lvl, i, t.m, t.n); */
        }
        else {
            fprintf(stderr, "??\n");
        }
...
int
testing_zrecgemm_R( run_arg_list_t *args, int check )
{
    ...
    int          nb     = run_arg_get_int( args, "nb", K );
    ...
    CHAMELEON_Recursive_Desc_Create_Full(
        &descA, CHAMELEON_MAT_ALLOC_GLOBAL, ChamComplexDouble, list_nb, list_nb,
...
testing_t   test_zrecgemm_R;
const char *zrecgemm_R_params[] = { "mtxfmt", "nb", "l1", "l2", "l3", "rec", "rarg", "transA", "transB", "m",     "n",     "k",     "lda", "ldb",
                                    "ldc", "alpha",  "beta",   "seedA", "seedB", "seedC", NULL };
const char *zrecgemm_R_output[] = { NULL };
const char *zrecgemm_R_outchk[] = { "RETURN", NULL };

/**
 * @brief Testing registration function
 */
void testing_zrecgemm_R_init( void ) __attribute__( ( constructor ) );
void
testing_zrecgemm_R_init( void )
{
    test_zrecgemm_R.name        = "zrecgemm_R";
    test_zrecgemm_R.helper      = "Outer-product";
    test_zrecgemm_R.params      = zrecgemm_R_params;
    test_zrecgemm_R.output      = zrecgemm_R_output;
    test_zrecgemm_R.outchk      = zrecgemm_R_outchk;
    test_zrecgemm_R.fptr_desc   = testing_zrecgemm_R;
    test_zrecgemm_R.fptr_std    = NULL;
    test_zrecgemm_R.next        = NULL;

    testing_register( &test_zrecgemm_R );
}
