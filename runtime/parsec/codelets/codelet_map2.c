/**
 *
 * @file parsec/codelet_map2.c
 *
 * @copyright 2018-2020 Bordeaux INP, CNRS (LaBRI UMR 5800), Inria,
 *                      Univ. Bordeaux. All rights reserved.
 *
 ***
 *
 * @brief Chameleon map2 PaRSEC codelet
 *
 * @version 1.0.0
 * @author Mathieu Faverge
 * @date 2020-03-03
 *
 */
#include "chameleon_parsec.h"
#include "chameleon/tasks.h"

static inline int
CORE_map2_parsec( parsec_execution_stream_t *context,
                  parsec_task_t             *this_task )
{
    const CHAM_desc_t *descA;
    const CHAM_desc_t *descB;
    cham_uplo_t uplo;
    int m;
    int n;
    void *dataA;
    void *dataB;
    cham_binary_operator_t op_fct;
    void *op_args;

    parsec_dtd_unpack_args(
        this_task, &uplo, &m, &n,
        &descA, &dataA, &descB, &dataB,
        &op_fct, &op_args );
    op_fct( uplo, m, n, descA, dataA, descB, dataB, op_args );

    (void)context;
    return PARSEC_HOOK_RETURN_DONE;
}

void INSERT_TASK_map2( const RUNTIME_option_t *options,
                       cham_uplo_t uplo,
                       const CHAM_desc_t *A, int Am, int An,
                       const CHAM_desc_t *B, int Bm, int Bn,
                       cham_binary_operator_t op_fct, void *op_args )
{
    parsec_taskpool_t* PARSEC_dtd_taskpool = (parsec_taskpool_t *)(options->sequence->schedopt);

    parsec_dtd_taskpool_insert_task(
        PARSEC_dtd_taskpool, CORE_map2_parsec, options->priority, "map2",
        sizeof(cham_uplo_t),              &uplo, VALUE,
        sizeof(int),                      &Am,   VALUE,
        sizeof(int),                      &An,   VALUE,
        sizeof(CHAM_desc_t*),             &A,    VALUE,
        PASSED_BY_REF, RTBLKADDR(A, void, Am, An), chameleon_parsec_get_arena_index( A ) | INPUT,
        sizeof(CHAM_desc_t*),             &B,    VALUE,
        PASSED_BY_REF, RTBLKADDR(B, void, Bm, Bn), chameleon_parsec_get_arena_index( B ) | INOUT,
        sizeof(cham_binary_operator_t),   &op_fct,  VALUE,
        sizeof(void*),                    &op_args, VALUE,
        PARSEC_DTD_ARG_END );
}
