...
static inline int
cl_zplrnt_is_bubble( struct starpu_task *t, void *_args )
{
    struct cl_zplrnt_args_s *clargs = (struct cl_zplrnt_args_s *)(t->cl_arg);
    (void)_args;
...
static void
cl_zplrnt_bubble_func( struct starpu_task *t, void *_args )
{
    struct cl_zplrnt_args_s *clargs  = (struct cl_zplrnt_args_s *)(t->cl_arg);
    ...
    chameleon_pzplrnt( clargs->tileA->mat,
                         ...
                         int bigM, int m0, int n0, unsigned long long int seed )
{
    ...
    if ( is_bubble ) {
        b_args = malloc( sizeof(bubble_args_t) + sizeof(struct cl_zplrnt_args_s) );
        b_args->sequence = options->sequence;
        b_args->parent   = request->parent;
        memcpy( &(b_args->clargs), clargs, sizeof(struct cl_zplrnt_args_s) );
        cl_name = "zplrnt_bubble";
    }
#endif
    ...
    rt_starpu_insert_task(
        ...
        STARPU_BUBBLE_GEN_DAG_FUNC,     cl_zplrnt_bubble_func,
