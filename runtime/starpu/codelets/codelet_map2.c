...
CHAMELEON_CL_CB(map2, cti_handle_get_m(task->handles[1]), cti_handle_get_n(task->handles[1]), 0, M*N)

#if !defined(CHAMELEON_SIMULATION)
static void cl_map2_cpu_func(void *descr[], void *cl_arg)
{
    ...
    starpu_codelet_unpack_args(cl_arg, &uplo, &m, &n, &descA, &descB, &op_fct, &op_args );
    op_fct( uplo, m, n,
            descA, tileA,
            descB, tileB,
            op_args );
 ...
 * Codelet definition
 */
CODELETS_CPU(map2, cl_map2_cpu_func)
                       ...
                       const char *name)
{
    ...
    rt_starpu_insert_task(
        ...
        cham_to_starpu_access(accessB),           RTBLKADDR(B, void, Bm, Bn),
        STARPU_VALUE,    &op_fct,                 sizeof(cham_binary_operator_t),
